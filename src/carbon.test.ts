...
let subject: Carbon;
beforeEach(() => (subject = Carbon.make("2020-01-01")));

test("#setLocale", () => {});

test("#isBefore", () => {
	expect(subject.isBefore(Carbon.make("2020-01-01").addDay())).toBeTrue();
	expect(subject.isBefore(Carbon.make("2020-01-01").subDay())).toBeFalse();
});

test("#isSame", () => {
	expect(subject.isSame(subject)).toBeTrue();
	expect(subject.isSame(Carbon.make("2020-01-01").addDay())).toBeFalse();
});

test("#isAfter", () => {
	expect(subject.isAfter(Carbon.make("2020-01-01").subDay())).toBeTrue();
	expect(subject.isAfter(Carbon.make("2020-01-01").addDay())).toBeFalse();
});
...
test("#format", () => {
	expect(subject.format("YYYY-MM-DDTHH:mm:ssZ[Z]")).toBe(
